...
$$P^\star = \arg\min_{\mathbb{P}^u \in \mathcal{P}(\mathcal{C}([0,T];\mathbb{R}^d))} \mathrm{KL}(P^u \| Q) \quad \text{s.t.} \quad P^u_0 = \pi_0,\; P^u_T = \pi_T$$

By Girsanov's theorem, the path-space KL divergence reduces to a kinetic energy cost over the control drift $u$:

$$\mathrm{KL}(\mathbb{P}^u \| \mathcal{Q}) = \mathbb{E}_{X_{0:T} \sim \mathbb{P}^u}\!\left[\int_0^T \frac{1}{2}\|u(X_t, t)\|^2\, dt\right]$$

---

## Wiener Prior

When $\mathbb{P} = \mathbb{W}^\epsilon$ with $f \equiv 0$ and $\sigma_t = \sqrt{\epsilon}$, the problem reduces to learning the **adjusted Schrödinger potential** $v^\star : \mathbb{R}^d \to \mathbb{R}_+$, which fully determines both the optimal coupling and the optimal drift.
