...
$$
\sigma_t \mathbf{u}_{v_\theta}(\mathbf{x}, t) = \epsilon \nabla_{\mathbf{x}} \log \left( \mathcal{N}\big(\mathbf{x} \mid 0, (T-t)\epsilon \mathbf{I}_d\big) \sum_{k=1}^{K} \alpha_k \mathcal{N}(\boldsymbol{\mu}_k \mid 0, \epsilon \boldsymbol{\Sigma}_k) \mathcal{N}\left( \boldsymbol{A}_k(t)^{-1} \boldsymbol{h}_k(\mathbf{x}, t) | 0, \boldsymbol{A}_k(t)^{-1} \right) \right)
$$
