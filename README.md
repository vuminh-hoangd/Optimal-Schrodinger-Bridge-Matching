...
**LightSB-M** parameterizes $v_\theta (\mathbf{x}_T) = \sum_{k=1}^K \alpha_k \mathcal{N}(\mathbf{x}_T \mid \mu_k, \epsilon \Sigma_k)$ as a Gaussian mixture and optimizes the bridge matching loss:
...
$$
\sigma_t \boldsymbol{u}_{\theta}(\mathbf{x}, t) = \epsilon \nabla_\mathbf{x} \log \left( \mathcal{N}(\mathbf{x} | 0, (T-t)\epsilon \mathbf{I}_d) \sum_{k=1}^{K} \alpha_k \mathcal{N}(\mu_k | 0, \epsilon \Sigma_k) \mathcal{N} \big( \boldsymbol{A}_k(t)^{-1} \boldsymbol{h}_k(t) \,\big|\, 0, \boldsymbol{A}_k(t)^{-1} \big)^{-1} \right)
$$

where $\boldsymbol{A}_k(t) := \frac{t}{T(T-t)\epsilon} \mathbf{I}_d + \frac{\Sigma_k^{-1}}{\epsilon}$, 
$\boldsymbol{h}_k(\mathbf{x}, t) := \frac{1}{\epsilon} \left( \frac{\mathbf{x}}{T-t} + \Sigma_k^{-1} \mu_k \right)$.
