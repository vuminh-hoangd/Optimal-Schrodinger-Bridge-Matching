...
Given two marginal distributions $\pi_0, \pi_T \in \mathcal{P}(\mathbb{R}^d)$ and a reference path measure $Q \in \mathcal{P}(C([0,T];\mathbb{R}^d))$ defined by the SDE $dX_t = f(X_t,t)\,dt + \sigma_t\,dB_t$, the **dynamic Schrödinger Bridge** seeks the path measure $P^\star$ of minimal KL divergence from $Q$ subject to the marginal constraints:
